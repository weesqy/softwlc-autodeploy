...
#   5) создание systemd-службы labcheck для автозапуска;
#   6) запуск службы и проверку доступности порта 9090.
#
...
#
# Если нужно принудительно установить labcheck до запуска EMS-сервера
# SoftWLC, проверку можно пропустить явно:
#     sudo SKIP_SOFTWLC_CHECK=1 ./install_labcheck_server.sh
# ============================================================

# Внутренний URL приложения (только если задан явно через окружение)
APP_URL="${APP_URL:-}"

# Принудительный обход проверки SoftWLC. По умолчанию отключён:
# labcheck зависит от баз данных SoftWLC на этом же сервере.
SKIP_SOFTWLC_CHECK="${SKIP_SOFTWLC_CHECK:-0}"
...
# SoftWLC (модуль EMS) слушает порт 8080 — это рабочий признак, что
...
# при обычном сценарии установки SoftWLC должен быть доступен заранее.
if [[ "$SKIP_SOFTWLC_CHECK" == "1" ]]; then
    log "[ВНИМАНИЕ] Проверка SoftWLC пропущена: SKIP_SOFTWLC_CHECK=1."
    log "            Продолжайте только если SoftWLC действительно установлен"
    log "            и будет доступен приложению labcheck на этом сервере."
elif ! command -v ss >/dev/null 2>&1; then
    fail "Не найдена утилита ss для проверки EMS-порта SoftWLC.
Установите пакет iproute2 или запустите сценарий с явным пропуском проверки:
  sudo SKIP_SOFTWLC_CHECK=1 $0"
...
поэтому установка без работающего SoftWLC обычно приведёт к нерабочему
развёртыванию. Сначала установите и запустите SoftWLC, затем повторите
запуск этого сценария.

Если SoftWLC установлен, но EMS ещё запускается после перезагрузки,
подождите несколько минут и повторите запуск. Для осознанной предварительной
установки labcheck проверку можно пропустить так:
  sudo SKIP_SOFTWLC_CHECK=1 $0"
fi

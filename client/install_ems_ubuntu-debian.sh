...
#
# Если sudo выдаёт «<пользователь> отсутствует в файле sudoers»:
#   Эта ошибка появляется ДО запуска скрипта — sudo блокирует выполнение,
#   потому что учётная запись не входит в группу sudo. Так бывает на Debian,
#   где первый пользователь не добавляется в эту группу автоматически.
#   Исправление от имени root (пароль root задаётся при установке ОС):
#     su -
#     usermod -aG sudo <пользователь>
#     exit
#   После этого выйдите из сеанса и войдите заново (или перезагрузитесь),
#   чтобы членство в группе вступило в силу, и повторите запуск через sudo.
#   Если пароль root неизвестен — то же самое можно сделать из режима
#   восстановления (recovery mode) в меню загрузчика GRUB.
# ============================================================
...
# Проверяет, что строка — корректный IPv4-адрес: ровно четыре октета,
# разделённых точками, каждый из которых — число от 0 до 255.
# Возвращает 0 (успех), если адрес корректен, иначе 1.
is_valid_ipv4() {
    local ip="$1"
    # Базовый формат: четыре группы из 1–3 цифр, разделённые точками.
    [[ "$ip" =~ ^[0-9]{1,3}(\.[0-9]{1,3}){3}$ ]] || return 1
    # Каждый октет должен находиться в диапазоне 0–255.
    local -a octets
    IFS='.' read -ra octets <<< "$ip"
    local o
    for o in "${octets[@]}"; do
        # 10# — явно десятичная система (защита от трактовки 08, 09 как восьмеричных).
...
# Примечание: если sudo сам отказывает с «<пользователь> отсутствует в файле
# sudoers», эта проверка не сработает — sudo блокирует запуск раньше. В таком
# случае добавьте учётную запись в группу sudo от имени root (см. шапку скрипта).
[[ $EUID -eq 0 ]] || fail "Запустите скрипт через sudo, например: sudo bash $0
Если sudo сообщает «<пользователь> отсутствует в файле sudoers», учётная запись
не входит в группу sudo (частый случай на Debian). Добавьте её от имени root:
    su -
    usermod -aG sudo <пользователь>
    exit
затем выйдите из сеанса и войдите заново и повторите запуск через sudo."
...
sudo $0 http://<ip-сервера>:8080/ems/jws"
    while true; do
        ...
        if ! is_valid_ipv4 "$SERVER_IP"; then
            echo "Некорректный IP-адрес (каждое число должно быть от 0 до 255), попробуйте ещё раз."
            continue
        fi
...
PROF
    ...
    # Пакет openjdk-17-jdk присутствует не во всех версиях ОС: его нет как на
    # слишком старых (Ubuntu 18.04, Debian 10), так и на более новых дистрибутивах
    # (например, Debian 13/trixie), где OpenJDK 17 убран из репозитория.
    # Проверяем доступность пакета до попытки установки, чтобы выдать понятное
    # пояснение вместо неочевидной ошибки apt.
    if ! apt-cache show openjdk-17-jdk >/dev/null 2>&1; then
        fail "В репозитории этой версии ОС нет пакета openjdk-17-jdk.
Так бывает на слишком старых дистрибутивах (Ubuntu 18.04, Debian 10), а также на
более новых, где OpenJDK 17 убран из репозитория (например, Debian 13 / trixie).
Перезапустите скрипт и выберите офлайн-режим (пункт 2): в нём оба компонента
ставятся из локальных, заранее скачанных архивов и не зависят от репозитория ОС.
Потребуются ДВА архива:
  - JDK 17 (.tar.gz), например OpenJDK17U-jdk_x64_linux_hotspot_17.0.14_7.tar.gz;
  - IcedTea-Web (icedtea-web-1.8.8.linux.bin.zip)."
    fi

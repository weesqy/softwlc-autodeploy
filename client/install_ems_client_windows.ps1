...
if (-not $JdkMsi -and -not $IcedTeaMsi) {
    ...
    if ($installMode -eq "2") {
        while ($true) {
            $JdkMsi = Read-Host "Укажите путь к установщику JDK 17 (jdk-17.0.12_windows-x64_bin.msi)"
            if (Test-Path $JdkMsi) { Write-Host "Установщик JDK найден."; break }
            Write-Host "Файл не найден: $JdkMsi. Попробуйте ещё раз."
        }
        while ($true) {
            $IcedTeaMsi = Read-Host "Укажите путь к установщику IcedTea-Web (icedtea-web-1.8.8.msi)"
            if (Test-Path $IcedTeaMsi) { Write-Host "Установщик IcedTea-Web найден."; break }
            Write-Host "Файл не найден: $IcedTeaMsi. Попробуйте ещё раз."
        }

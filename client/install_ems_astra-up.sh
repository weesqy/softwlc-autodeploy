...
# Целевая ОС: семейство Astra Linux (Special Edition Smolensk 1.6/1.7/1.8,
#             Common Edition Orel 2.12), архитектура x86-64.
#
# Отличия от сценария для Ubuntu:
#   - JDK 17 устанавливается из архива (tar.gz), так как в штатном
#     репозитории Astra Linux нет подходящей версии (этот подход не
#     зависит от редакции и работает на всём ряду версий);
...
#
# Особенности Astra, учтённые в сценарии:
...
#   - при включённой замкнутой программной среде (ЗПС) выводится
#     предупреждение: неподписанные бинарники Temurin и IcedTea-Web
#     могут быть заблокированы модулем ядра digsig_verif.
#
# Использование:
...
# Признак включённой ЗПС (заполняется проверкой ниже).
ZPS_ON=0
...
# Извлекает значение параметра KEY=VALUE из конфигурационного файла
# (берётся последнее присвоение), отбрасывая пробелы и комментарий
# после значения. Всегда завершается успешно.
get_conf_val() {
...
# Определяет, включён ли в системе режим замкнутой программной среды (ЗПС).
# ЗПС задаётся в /etc/digsig/digsig_initramfs.conf параметром DIGSIG_ELF_MODE
# (значение, отличное от 0); в выпусках 1.5/1.6 — параметром DIGSIG_ENFORCE.
# Возвращает 0, если ЗПС включена, иначе 1.
zps_enabled() {
    local conf="/etc/digsig/digsig_initramfs.conf"
    [[ -f "$conf" ]] || return 1
    local elf_mode enforce
    elf_mode="$(get_conf_val DIGSIG_ELF_MODE "$conf")"
    enforce="$(get_conf_val DIGSIG_ENFORCE "$conf")"
    if { [[ -n "$elf_mode" ]] && [[ "$elf_mode" != "0" ]]; } \
       || { [[ -n "$enforce" ]] && [[ "$enforce" != "0" ]]; }; then
        return 0
    fi
    return 1
}
...
# Проверка замкнутой программной среды. В режиме ЗПС модуль ядра digsig_verif
# блокирует запуск ELF-файлов без действительной встроенной ЭЦП. Распакованные
# из архивов бинарники Temurin JDK и IcedTea-Web не подписаны ключами этой
# системы, поэтому при включённой ЗПС они, скорее всего, не запустятся.
if zps_enabled; then
    ZPS_ON=1
    echo ""
    log "[ВНИМАНИЕ] На системе включена замкнутая программная среда (ЗПС)."
    log "           Модуль ядра digsig_verif блокирует запуск ELF-файлов без действительной"
    log "           встроенной ЭЦП. Бинарные файлы Temurin JDK и IcedTea-Web из архивов не"
    log "           подписаны ключами этой системы, поэтому после установки java и javaws,"
    log "           скорее всего, завершатся ошибкой проверки ЭЦП."
    log "           Возможные решения:"
    log "             - подписать бинарники штатными средствами Astra (bsign / fly-admin-smc),"
    log "               поместив открытый ключ в /etc/digsig/keys, и пересобрать initramfs;"
    log "             - использовать ПО из подписанного репозитория Astra Linux;"
    log "             - для отладки временно отключить ЗПС: sudo astra-digsig-control disable"
    log "               (с последующей перезагрузкой системы)."
    if [[ -e /dev/tty ]]; then
        read -rp "Продолжить установку, несмотря на включённую ЗПС? [Y/n]: " ZPS_ANSWER </dev/tty
        [[ "$ZPS_ANSWER" =~ ^[Nn]$ ]] && fail "Установка прервана пользователем из-за включённой ЗПС."
    fi
fi
...
# Проверка запуска java. При включённой ЗПС именно здесь, как правило,
# проявляется блокировка неподписанного исполняемого файла.
if ! java -version; then
    if [[ "$ZPS_ON" -eq 1 ]]; then
        fail "Команда 'java -version' завершилась ошибкой при включённой ЗПС.
Вероятно, модуль digsig_verif заблокировал запуск неподписанного исполняемого файла java.
Подпишите бинарники JDK средствами Astra (bsign / fly-admin-smc) либо временно отключите ЗПС
для тестирования: sudo astra-digsig-control disable (с последующей перезагрузкой)."
    else

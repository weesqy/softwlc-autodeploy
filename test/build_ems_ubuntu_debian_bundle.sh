#!/usr/bin/env bash
#
# build_ems_ubuntu_debian_bundle.sh — сборка офлайн-архива EMS-клиента SoftWLC для Ubuntu/Debian.
#
...
#
# Результат:
#   ems-ubuntu-debian-client-offline.tar.gz
#
# Состав архива:
#   install_ems_ubuntu-debian.sh
...
#   SHA256SUMS
#   README_OFFLINE_UBUNTU_DEBIAN.txt
#
...
INSTALL_SCRIPT="${HERE}/install_ems_ubuntu-debian.sh"
OUT="${OUT:-${HERE}/ems-ubuntu-debian-client-offline.tar.gz}"
WORK="$(mktemp -d)"
KIT="${WORK}/kit"
...
log()  { echo "[build][ubuntu-debian] $*"; }
fail() { echo "[build][ubuntu-debian][ОШИБКА] $*" >&2; exit 1; }
...
find_single_match() {
  ...
  if [[ ${#matches[@]} -eq 1 ]]; then
    ...
    {
      echo "[build][ubuntu-debian][ОШИБКА] Найдено несколько файлов для ${label}:"
      printf '  - %s\n' "${matches[@]}"
...
# Разрешаем файлы с суффиксами вида install_ems_ubuntu-debian(6).sh,
# если они были скачаны из браузера/мессенджера. В архив кладём нормальное имя.
if [[ ! -f "$INSTALL_SCRIPT" ]]; then
  INSTALL_SCRIPT="$(find_single_match 'install_ems_ubuntu-debian*.sh' 'клиентский сценарий Ubuntu/Debian')"
fi

mkdir -p "${KIT}/deps"

log "Копирование сценария установки Ubuntu/Debian..."
cp "$INSTALL_SCRIPT" "${KIT}/install_ems_ubuntu-debian.sh"
chmod +x "${KIT}/install_ems_ubuntu-debian.sh"

if command -v bash >/dev/null 2>&1; then
  log "Проверка синтаксиса install_ems_ubuntu-debian.sh..."
  bash -n "${KIT}/install_ems_ubuntu-debian.sh" \
    || fail "В install_ems_ubuntu-debian.sh обнаружена синтаксическая ошибка."
fi
...
cat > "${KIT}/README_OFFLINE_UBUNTU_DEBIAN.txt" <<'README'
Офлайн-комплект EMS-клиента SoftWLC для Ubuntu/Debian
=====================================================
...
Как использовать на клиентской машине Ubuntu/Debian:

1. Распаковать архив:
     tar -xzf ems-ubuntu-debian-client-offline.tar.gz
...
Примечания:
  - Комплект рассчитан на клиентские машины x86-64/amd64.
...
(
  ...
  sha256sum \
    ...
    "deps/${JDK_FILE}" \
    "deps/${ICEDTEA_FILE}" \
    README_OFFLINE_UBUNTU_DEBIAN.txt > SHA256SUMS
)
...
for required in \
  ...
  "${KIT}/SHA256SUMS" \
  "${KIT}/README_OFFLINE_UBUNTU_DEBIAN.txt"; do
...
log "Готово: ${OUT} (${SIZE})."
log "Дальше: скопируйте архив на клиент Ubuntu/Debian, распакуйте, запустите install_ems_ubuntu-debian.sh и выберите офлайн-режим."
